...
extern "C" void XDKSetGamma(unsigned char * red, unsigned char * green, unsigned char * blue) 
{
	D3DGAMMARAMP gamma;
	memcpy(gamma.red, red, 256);
	memcpy(gamma.green, green, 256);
	memcpy(gamma.blue, blue, 256);

	GLImpl.device->SetGammaRamp(0, 0, &gamma);
}